...
/**
 * 전부 여행 일정에서부터 로그인을 하는게 아니니깐  redirect 있을떄만 지금처럼하게 바꿔야함
 */
 const handleLogin = async () => {
  try {
    // 서버에 로그인 (쿠키는 HttpOnly로 세팅됨)
    await api.post('members/sign-in', {
      username: email,   // 🔹 email 값을 username 필드로 매핑
      password: password
    });

    // redirect 쿼리 있으면 그쪽으로, 없으면 이전 화면(혹은 기본 경로)
